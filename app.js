...
yargs.command({
  ...
  handler({ period, openAssets, assetsYear, growthRate, openDepreciation }) {
    calculations(period, openAssets, assetsYear, growthRate, openDepreciation);
  }
}).check(({ openAssets, assetsYear, growthRate }) => {
  if (isNaN(openAssets) || isNaN(assetsYear) || isNaN(growthRate)) {
    throw new Error(chalk.red('Next arguments: openAssets, assetsYear, growthRate must be numbers'));
  } else {
    return true;
  }
});
